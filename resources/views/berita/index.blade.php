...
<main class="pt-28 md:pt-32 bg-gray-50">
    <!-- Header Berita -->
    <div class="bg-green-700 text-white py-8 px-4 md:px-20">
        <h1 class="text-3xl md:text-4xl font-bold">Berita Terkini</h1>
        <p class="mt-2">Update terbaru seputar informasi di DMI Provinsi Jawa Tengah</p>
    </div>
    ...
    <div class="container mx-auto px-4 md:px-6 py-8">
        ...
        <div class="mb-8 bg-white p-4 rounded-lg shadow-md">
            <form action="{{ route('berita.publicIndex') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-center">
                ...
                <div class="w-full md:w-48 relative">
                    ...
                    <select name="tag" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 appearance-none">
                ...
                <div class="w-full md:w-48 relative">
                    ...
                    <select name="sort" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 appearance-none">
                        <option value="newest" {{ request('sort') == 'newest' ? 'selected' : '' }}>Terbaru</option>
                        <option value="oldest" {{ request('sort') == 'oldest' ? 'selected' : '' }}>Terlama</option>
                    </select>
                </div>
                
                <button type="submit" class="flex items-center bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-6 rounded-lg transition duration-300">
                ...
                <div class="md:flex">
                    ...
                    <div class="p-6 md:w-1/3">
                        <div class="flex justify-between items-center mb-2">
                            <span class="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 rounded">{{ $beritas[0]->tag ?? 'Headline' }}</span>
                        ...
                        <a href="{{ route('berita.show', $beritas[0]->id) }}" class="text-green-600 font-semibold hover:underline">Baca Selengkapnya →</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                ...
                <div class="p-4">
                    ...
                    <p class="text-gray-600 text-sm mb-3 line-clamp-2">{{ Str::limit(strip_tags($berita->content), 100) }}</p>
                    <div class="flex justify-between items-center">
                        <span class="text-xs text-gray-500">{{ $berita->author_name ?? 'Admin DMI' }}</span>
                        <a href="{{ route('berita.show', $berita->id) }}" class="text-green-600 text-sm font-medium hover:underline">Baca →</a>
                    </div>
                </div>
            </div>
            @endforeach
